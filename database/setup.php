...
function ensure_tables(mysqli $conn): void
{
    ...
    if (!$columnExists($conn, 'coupon_usages', 'guest_identity_hash')) {
        $conn->query("ALTER TABLE coupon_usages ADD COLUMN guest_identity_hash CHAR(64) NULL AFTER customer_id");
    }
    if (!$indexExists($conn, 'coupon_usages', 'uq_coupon_usages_coupon_guest')) {
        $conn->query("ALTER TABLE coupon_usages ADD UNIQUE KEY uq_coupon_usages_coupon_guest (coupon_id, guest_identity_hash)");
    }
    $conn->query(
