...
<form method="POST" class="l-grid l-grid--12 u-gap-3" id="product-editor-form" data-initial-editor-section="<?php echo e($initialEditorSection); ?>">
 ...
 <div class="l-col-md-third" data-editor-section="details"><label class="ui-label" for="unit_type">Selling Unit</label><select id="unit_type" name="unit_type" class="<?php echo form_class($errors,'unit_type','ui-select');?>" required><option value="piece" <?php echo ($old['unit_type']??'')==='piece'?'selected':'';?>>Piece</option><option value="set" <?php echo ($old['unit_type']??'')==='set'?'selected':'';?>>Set</option><option value="meter" <?php echo ($old['unit_type']??'')==='meter'?'selected':'';?>>Meter</option></select><?php echo form_error($errors,'unit_type');?><div class="ui-help">Changing the unit moves the product to draft so inventory can be reviewed.</div></div>
 ...
 <div class="l-col-md-third" data-editor-section="details"><label for="category" class="ui-label">Product Type *</label><select id="category" name="category" required class="<?php echo form_class($errors,'category','ui-select');?>"><option value="">Select product type</option><?php foreach($categories as $cat):?><option value="<?php echo e((string)$cat['slug']);?>" data-default-unit-type="<?php echo e((string)($cat['default_unit_type']??''));?>" <?php echo ($old['category']??'')===$cat['slug']?'selected':'';?>><?php echo e((string)$cat['name']);?></option><?php endforeach;?></select><?php echo form_error($errors,'category');?></div>
 ...
 <div class="l-col-md-quarter" data-editor-section="pricing"><label for="selling_price" class="ui-label">Selling Price<span data-meter-price-suffix><?php echo ($old['unit_type']??'')==='meter'?' per Meter':'';?></span> *</label><input id="selling_price" type="number" name="selling_price" min="0.01" step="0.01" required class="<?php echo form_class($errors,'sale_price');?>" value="<?php echo e((string)($old['selling_price']??''));?>"><?php echo form_error($errors,'sale_price');?></div>
 <div class="l-col-md-quarter" data-editor-section="pricing"><label for="mrp" class="ui-label">MRP<span data-meter-price-suffix><?php echo ($old['unit_type']??'')==='meter'?' per Meter':'';?></span> *</label><input id="mrp" type="number" name="mrp" min="0.01" step="0.01" required class="<?php echo form_class($errors,'price');?>" value="<?php echo e((string)($old['mrp']??''));?>"><?php echo form_error($errors,'price');?></div>
 ...
 <div class="l-col-md-quarter" data-editor-section="pricing"><label for="quantity" class="ui-label">Stock Quantity<span id="quantity_unit_suffix"><?php echo ($old['unit_type']??'')==='meter'?' (meters)':' (units)';?></span> *</label><input id="quantity" type="number" name="quantity" min="0" step="<?php echo ($old['unit_type']??'')==='meter'?'0.01':'1';?>" required class="<?php echo form_class($errors,'stock');?>" value="<?php echo e((string)($old['quantity']??0));?>"><?php echo form_error($errors,'stock');?></div>
 <div class="l-col-md-half" data-editor-section="pricing" data-meter-unit-field <?php echo ($old['unit_type']??'')==='meter'?'':'hidden';?>><label for="meter_options" class="ui-label">Meter Length Options *</label><input id="meter_options" name="meter_options" data-meter-required class="<?php echo form_class($errors,'meter_options');?>" value="<?php echo e((string)($old['meter_options']??''));?>" placeholder="10, 20, 30" <?php echo ($old['unit_type']??'')==='meter'?'required':'';?>><div class="ui-help">Comma-separated customer choices. Price is calculated per meter × selected length × bundle quantity.</div><?php echo form_error($errors,'meter_options');?></div>
