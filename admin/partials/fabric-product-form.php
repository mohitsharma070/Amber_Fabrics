...
$initialEditorTarget='';
if(!empty($errors)){
 $pricingErrorFields=['sale_price','price','cost_price','stock','quantity','meter_options','qty_step','gst_rate','hsn_code','shipping_weight_kg','parcel_length_cm','parcel_width_cm','parcel_height_cm'];
...
<form method="POST" class="row g-3" id="product-editor-form" data-initial-editor-section="<?php echo e($initialEditorSection); ?>" data-initial-editor-target="<?php echo e($initialEditorTarget); ?>">
 ...
 <div class="col-md-3" data-editor-section="pricing" data-meter-unit-field <?php echo ($old['unit_type']??'')==='meter'?'':'hidden';?>><label for="qty_step" class="form-label">Meter Quantity Step</label><input id="qty_step" type="number" name="qty_step" min="0.01" step="0.01" class="<?php echo form_class($errors,'qty_step');?>" value="<?php echo e((string)($old['qty_step']??''));?>" placeholder="e.g. 0.25"><div class="form-text">Meter steps support up to two decimal places. Leave blank for the default step.</div><?php echo form_error($errors,'qty_step');?></div>
