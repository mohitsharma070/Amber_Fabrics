...
<div class="l-col-md-third"><label for="name" class="ui-label">Name</label><input id="name" class="ui-input" name="name" maxlength="255" required></div>
<div class="l-col-md-third"><label for="email" class="ui-label">Email</label><input id="email" class="ui-input" type="email" name="email" maxlength="255" required></div>
<div class="l-col-md-quarter"><label for="role" class="ui-label">Role</label><select id="role" class="ui-select" name="role"><?php foreach ($roles as $option): ?><option value="<?php echo e($option); ?>"><?php echo e(ucwords(str_replace('_', ' ', $option))); ?></option><?php endforeach; ?></select></div>
...
<td><form id="<?php echo e($rowFormId); ?>" method="POST"><?php echo csrf_field(); ?><input type="hidden" name="action" value="update"><input type="hidden" name="admin_id" value="<?php echo (int) $row['id']; ?>"></form><input class="ui-input ui-input--small" name="name" aria-label="Admin name" form="<?php echo e($rowFormId); ?>" maxlength="255" value="<?php echo e((string) $row['name']); ?>" required></td>
<td><input class="ui-input ui-input--small" type="email" name="email" aria-label="Admin email" form="<?php echo e($rowFormId); ?>" maxlength="255" value="<?php echo e((string) $row['email']); ?>" required></td>
<td><select class="ui-select ui-select--small" name="role" aria-label="Admin role" form="<?php echo e($rowFormId); ?>"><?php foreach ($roles as $option): ?><option value="<?php echo e($option); ?>" <?php echo $option === (string) $row['role'] ? 'selected' : ''; ?>><?php echo e(ucwords(str_replace('_', ' ', $option))); ?></option><?php endforeach; ?></select></td>
