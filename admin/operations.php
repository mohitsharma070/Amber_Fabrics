...
?>
<div class="mb-4"><h1 class="h3 mb-1">Operations Center</h1><p class="text-muted mb-0">Sanitized, read-only operational history. Provider signatures and raw payloads are never displayed.</p></div>
<ul class="nav nav-tabs mb-3"><?php foreach ($allowedTabs as $item): ?><li class="nav-item"><a class="nav-link <?php echo $tab === $item ? 'active' : ''; ?>" href="operations.php?tab=<?php echo e($item); ?>"><?php echo e(ucfirst($item)); ?></a></li><?php endforeach; ?></ul>
<form method="GET" class="row g-2 mb-3"><input type="hidden" name="tab" value="<?php echo e($tab); ?>"><div class="col-md-3"><input class="form-control" name="q" maxlength="100" placeholder="Order, product, provider or admin" value="<?php echo e($searchFilter); ?>"></div><div class="col-md-2"><input class="form-control" name="status" maxlength="40" placeholder="Status or movement" value="<?php echo e($statusFilter); ?>"></div><div class="col-md-2"><input type="date" class="form-control" name="from" value="<?php echo e($fromFilter); ?>" aria-label="From date"></div><div class="col-md-2"><input type="date" class="form-control" name="to" value="<?php echo e($toFilter); ?>" aria-label="To date"></div><div class="col-auto"><button class="btn btn-outline-primary">Filter</button></div><div class="col-auto"><a class="btn btn-outline-secondary" href="operations.php?tab=<?php echo e($tab); ?>">Reset</a></div></form>
<div class="table-responsive"><table class="table align-middle"><thead><tr><?php foreach ($columns as $column): ?><th><?php echo e($column); ?></th><?php endforeach; ?></tr></thead><tbody>
<?php if (!$rows): ?><tr><td colspan="<?php echo max(1, count($columns)); ?>" class="text-center text-muted py-4">No records found.</td></tr><?php endif; ?>
<?php foreach ($rows as $row): ?><tr>
<?php if ($tab === 'cron'): $details = json_decode((string) ($row['summary_json'] ?? '[]'), true); $details = is_array($details) ? $details : []; ?><td class="small"><?php echo e((string) $row['started_at']); ?><br><?php echo e((string) $row['finished_at']); ?></td><td><?php echo e((string) $row['status']); ?></td><td><?php echo (int) $row['jobs_total']; ?> total<br><span class="text-danger"><?php echo (int) $row['jobs_failed']; ?> failed</span> / <span class="text-warning"><?php echo (int) $row['jobs_degraded']; ?> degraded</span></td><td><?php echo (int) $row['duration_ms']; ?> ms</td><td class="small"><?php if (!$details): ?>—<?php else: ?><?php foreach ($details as $detail): ?><div><strong><?php echo e((string) ($detail['job'] ?? 'unknown')); ?></strong>: <?php echo e(CronService::sanitizeError((string) ($detail['error'] ?? $detail['status'] ?? ''))); ?><?php foreach ((array) ($detail['callbacks'] ?? []) as $callback): ?><br><span class="ms-2"><?php echo e((string) ($callback['callback'] ?? 'callback')); ?>: <?php echo e(CronService::sanitizeError((string) ($callback['error'] ?? $callback['status'] ?? ''))); ?></span><?php endforeach; ?></div><?php endforeach; ?><?php endif; ?></td>
<?php elseif ($tab === 'payments'): ?><td><?php echo e((string) ($row['order_number'] ?? '—')); ?></td><td><?php echo e((string) $row['provider']); ?><br><span class="small text-muted"><?php echo e((string) $row['attempt_ref']); ?></span></td><td><?php echo e((string) $row['status']); ?><br><span class="small"><?php echo e((string) $row['source']); ?></span></td><td><?php echo e((string) ($row['gateway_payment_id'] ?: '—')); ?></td><td class="small"><?php echo e(CronService::sanitizeError(trim((string) ($row['error_code'] ?? '') . ' ' . (string) ($row['error_message'] ?? '')))); ?></td><td><?php echo e((string) $row['last_seen_at']); ?></td>
<?php elseif ($tab === 'refunds'): ?><td><?php echo e((string) ($row['order_number'] ?? $row['order_id'])); ?></td><td><?php echo e(money((float) $row['amount'], (string) $row['currency'])); ?></td><td><?php echo e((string) $row['status']); ?></td><td><?php echo e((string) ($row['gateway'] ?? '')); ?><br><span class="small"><?php echo e((string) ($row['gateway_refund_id'] ?? '')); ?></span></td><td class="small"><?php echo e(CronService::sanitizeError((string) ($row['notes'] ?? ''))); ?></td><td><?php echo e((string) $row['created_at']); ?></td>
<?php elseif ($tab === 'stock'): ?><td><?php echo e((string) ($row['product_name'] ?? 'Product')); ?><br><span class="small"><?php echo e((string) ($row['variant_sku'] ?? '')); ?></span></td><td><?php echo e((string) ($row['order_number'] ?? '—')); ?></td><td><?php echo e((string) $row['movement']); ?> / <?php echo e((string) $row['direction']); ?></td><td><?php echo e(format_quantity_by_unit((float) $row['quantity'], (string) $row['unit_type'])); ?></td><td><?php echo e((string) ($row['source'] ?? '')); ?></td><td><?php echo e((string) $row['created_at']); ?></td>
<?php else: ?><td><?php echo e((string) $row['admin_name']); ?><br><span class="small"><?php echo e((string) $row['admin_email']); ?></span></td><td><?php echo e((string) $row['action']); ?></td><td><?php echo e((string) ($row['target_type'] ?? '')); ?> #<?php echo (int) ($row['target_id'] ?? 0); ?><br><span class="small"><?php echo e((string) ($row['route'] ?? '')); ?></span></td><td><?php echo e((string) $row['status']); ?></td><td class="small"><?php echo e(CronService::sanitizeError((string) ($row['details'] ?? ''))); ?></td><td><?php echo e((string) $row['created_at']); ?></td><?php endif; ?>
</tr><?php endforeach; ?>
</tbody></table></div>
