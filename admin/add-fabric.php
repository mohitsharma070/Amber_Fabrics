...
<div class="admin-narrow-page u-mx-auto">
  ...
  <form method="post" class="ui-card u-shadow js-no-loading" id="product-draft-form">
    <div class="ui-card__body l-grid l-grid--12 u-gap-3"><?php echo csrf_field(); ?>
      ...
      <div class="l-col-md-half"><label for="category" class="ui-label">Product Type *</label><select id="category" name="category" required class="<?php echo form_class($errors,'category','ui-select');?>"><option value="">Select product type</option><?php foreach($categories as $cat):?><option value="<?php echo e($cat['slug']);?>" <?php echo $old['category']===$cat['slug']?'selected':'';?>><?php echo e($cat['name']);?></option><?php endforeach;?></select><?php echo form_error($errors,'category');?></div>
      ...
      <div class="l-col-md-half"><label for="unit_type" class="ui-label">Selling Unit *</label><select name="unit_type" required id="unit_type" class="<?php echo form_class($errors,'unit_type','ui-select');?>"><option value="piece" <?php echo $old['unit_type']==='piece'?'selected':'';?>>Piece</option><option value="set" <?php echo $old['unit_type']==='set'?'selected':'';?>>Set</option><option value="meter" <?php echo $old['unit_type']==='meter'?'selected':'';?>>Meter</option></select><?php echo form_error($errors,'unit_type');?><div class="ui-help">The selling unit is locked after draft creation to protect inventory and order history.</div></div>
    </div>
