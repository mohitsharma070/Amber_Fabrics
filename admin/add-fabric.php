...
<div class="mx-auto" style="max-width:760px">
  ...
  <form method="post" class="card shadow-sm js-no-loading" id="product-draft-form">
    <div class="card-body row g-3"><?php echo csrf_field(); ?>
      ...
      <div class="col-md-6"><label for="category" class="form-label">Product Type *</label><select id="category" name="category" required class="<?php echo form_class($errors,'category','form-select');?>"><option value="">Select product type</option><?php foreach($categories as $cat):?><option value="<?php echo e($cat['slug']);?>" data-default-unit-type="<?php echo e((string)($cat['default_unit_type']??''));?>" <?php echo $old['category']===$cat['slug']?'selected':'';?>><?php echo e($cat['name']);?></option><?php endforeach;?></select><?php echo form_error($errors,'category');?></div>
      ...
      <div class="col-md-6"><label for="unit_type" class="form-label">Selling Unit *</label><select id="unit_type" name="unit_type" required class="<?php echo form_class($errors,'unit_type','form-select');?>"><option value="piece" <?php echo $old['unit_type']==='piece'?'selected':'';?>>Piece</option><option value="set" <?php echo $old['unit_type']==='set'?'selected':'';?>>Set</option><option value="meter" <?php echo $old['unit_type']==='meter'?'selected':'';?>>Meter</option></select><?php echo form_error($errors,'unit_type');?><div class="form-text">Categories can enforce a selling unit. You can change it later from the product editor when the category permits.</div></div>
    </div>
