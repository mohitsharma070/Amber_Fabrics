...
The cart is session-backed and can persist for authenticated customers. Cart updates must resolve an exact existing line before mutation: canonical `{product_id}::{variant_id}` keys are preferred, preserved legacy simple-product keys remain updateable, and the `product_id` compatibility fallback can resolve only an already-existing canonical line. `CartService` owns the sellable quantity grid: whole-unit quantities are MOQ-relative step values, while meter quantities use 10,000-scaled arithmetic and must also land on the two-decimal storage grid and the selected cut-length grid. Explicit add/update requests that miss the grid are rejected; stored hydration and compatible cart/wishlist/login merges may reconcile downward to stock with a customer notice. `CheckoutInput` owns checkout request normalization and validation, `CheckoutReadService` owns customer/last-order prefill and saved-address selection, `OrderItemSnapshotService` locks and snapshots canonical product/variant state, and `CheckoutPricingService` owns tax jurisdiction and inclusive-GST allocation. `OrderPersistenceService` owns the order, item, delivery-estimate, initial-payment, zero-amount, and quoted-shipment statements without starting or completing transactions. `CouponService` owns coupon normalization, validation, guest identity hashing, locked checkout resolution, reservation, and release; the established global coupon functions remain compatibility wrappers. `place-order.php` deliberately remains the transaction owner for order creation, inventory and coupon reservation, outbox enqueue, and commit. Checkout validates delivery details, coupon state, payment method, inventory, and a short-lived shipping quote token. `shipping-rate.php` calculates manual or courier-filtered rates. Its server-only `storefront_rate_request` hook context gives Bigship one monotonic 7-second budget across reference lookups, login, rate attempts, retry backoff and throttle waits; cURL uses the remaining milliseconds for connect/total timeouts. Timeout, provider error (including 429/5xx) or no usable rate keeps the canonical manual quote and stores its normal server quote token without returning provider error details. Checkout retains its 10-second browser timeout, token validation and stale-response guards. Admin rate tests, no-JavaScript checkout, cron and shipment operations retain their existing provider deadlines; if canonical cart hydration detects a removed or adjusted line, it returns a conflict without issuing a quote so checkout reloads and displays the repaired cart before the customer continues. The no-JavaScript checkout path contains courier exceptions and still signs the server-calculated manual quote. Delivery estimates are snapshotted on orders.

### Orders and payments

Order decisions and read models use the canonical `total_amount`, `shipping_amount`, `discount_amount`, `payment_status`, `order_status`, and `order_notes` fields. Legacy order columns remain compatibility mirrors written by the same statements.
...
Tests are standalone PHP programs invoked by `composer test`. They are source and invariant contracts so they can run without database credentials or third-party networks. They cover routing, shipping payloads, checkout state, product editing/import/variants, backend integrity, frontend regression markers, and compatibility-safe cleanup boundaries. The guarded Playwright commerce suite uses the disposable MySQL fixture and real first-party cart, wishlist, coupon, shipping, checkout, order, and persistence paths. A CLI probe verifies order items, payment state, coupon reservation, and inventory reservation. Only Razorpay's external API is replaced by a loopback stub, and browser tests block real payment, courier, and analytics hosts.
