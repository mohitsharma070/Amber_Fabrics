...
$orderItemSnapshot = (string) file_get_contents($root . '/includes/services/OrderItemSnapshotService.php');
...
$interactionLayer = (string) file_get_contents($root . '/includes/partials/interaction-layer-v2.php');
...
$assert(str_contains($editor, 'Base gallery') && str_contains($editor, 'Inherited') && str_contains($adminScript, 'effective_stock'), 'Variant editor must show inherited media, pricing, and unit-aware stock.');
$assert(str_contains($editor, 'variant-save-btn') && str_contains($adminScript, 'AmberUI.setButtonLoading(save, false)'), 'Variant editor must prevent double saves and restore the save state.');
foreach ([$addToCart, $cart, $order . $orderItemSnapshot, $estimate] as $source) {
...
$assert(str_contains($editor, 'variant-editor-modal') && str_contains($editor, 'data-ui-dialog') && str_contains($adminScript, '"sort_order"'), 'Variant add/edit must use a complete first-party dialog editor.');
$assert(str_contains($editor, 'data-ui-dialog-static') && !str_contains($editor, 'modal-dialog-scrollable'), 'Variant dialog must use a static first-party overlay without nested Bootstrap scroll behavior.');
$assert(str_contains($adminFooter, 'interaction-layer-v2.php') && str_contains($interactionLayer, 'data-ui-confirm-dialog') && str_contains($adminScript, 'AmberUI.openDialog') && !str_contains($editor, "Promise.resolve(confirm("), 'Admin confirmations must use the shared CSP-safe branded dialog.');
