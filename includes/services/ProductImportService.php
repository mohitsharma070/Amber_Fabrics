...
final class ProductImportService
{
    ...
    private static function validateRow(mysqli $conn,array $row,array $categories,string $defaultUnit,?array $existing=null): array
    {
        ...
        if($unit==='meter'&&$updating){
            ...
            if($existingStep!==''&&(!is_numeric($existingStep)||((float)$existingStep!=0.0&&!meter_qty_step_is_representable($existingStep))))$errors[]='Existing Meter Quantity Step must use no more than four decimal places. Correct it in the product editor before importing this row.';
        }
