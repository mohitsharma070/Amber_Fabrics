    ...
    if(!verify_csrf()||!is_array($tokenRow)){$errors[]='This activation session expired.';}else{
        ...
        if(!$errors){
            ...
            if($customerId>0&&!$errors){
                ...
                // The session cart is now persisted against this new customer, but activation
                // redirects to login without logging in. Mark ownership so the login-time merge
                // does not sum the same lines a second time.
...
<section class="page-hero"><div class="l-container"><h1>Activate Account</h1></div></section><section class="section-block"><div class="l-container"><div class="surface-panel u-p-4 u-mx-auto auth-card"><?php foreach($errors as $error):?><div class="ui-alert ui-alert--error"><?php echo e($error);?></div><?php endforeach;?><?php if(is_array($tokenRow)):?><form method="post"><?php echo csrf_field();?><div class="u-mb-3"><label for="password" class="ui-label">Password</label><input id="password" class="ui-input" type="password" name="password" required autocomplete="new-password"></div><div class="u-mb-3"><label for="confirm_password" class="ui-label">Confirm password</label><input id="confirm_password" class="ui-input" type="password" name="confirm_password" required autocomplete="new-password"></div><button class="ui-button ui-button--primary u-w-full">Activate Account</button></form><?php else:?><p class="u-mb-3">This activation link is invalid or expired.</p><a class="ui-button ui-button--outline" href="/guest/order-access">Manage your order</a><?php endif;?></div></div></section><?php include __DIR__.'/../includes/footer.php';?>
