...
<div class="surface-panel u-p-4 u-mt-4"><h5>Request a Refund Return</h5><p class="u-text-small u-text-muted">Available within <?php echo return_request_window_days(); ?> calendar days of confirmed delivery.</p><form method="post" action="/customer/request-return.php" enctype="multipart/form-data"><?php echo csrf_field(); ?><input type="hidden" name="order_id" value="<?php echo $orderId; ?>"><?php foreach ($items as $item): ?><label class="ui-label u-text-small" for="return-qty-<?php echo (int) $item['id']; ?>"><?php echo e((string) ($item['fabric_name_snapshot'] ?? 'Product')); ?> quantity</label><input id="return-qty-<?php echo (int) $item['id']; ?>" class="ui-input u-mb-2" type="number" step="<?php echo ($item['unit_type'] ?? 'meter') === 'meter' ? '0.01' : '1'; ?>" min="0" max="<?php echo e((string) (($item['quantity'] ?? 0) > 0 ? $item['quantity'] : $item['quantity_meters'])); ?>" name="return_qty[<?php echo (int) $item['id']; ?>]" value="0"><?php endforeach; ?><select class="ui-select u-mb-2" name="reason" aria-label="Return reason" required><option value="">Reason</option><option>Damaged Item</option><option>Wrong Item Delivered</option><option>Quality Not as Expected</option><option>Other</option></select><textarea class="ui-input u-mb-2" name="customer_note" placeholder="Optional note" aria-label="Additional details about the return"></textarea><input class="ui-input u-mb-2" type="file" name="image_1" accept="image/jpeg,image/png,image/webp" aria-label="Issue photo 1" required><input class="ui-input u-mb-2" type="file" name="image_2" accept="image/jpeg,image/png,image/webp" aria-label="Issue photo 2" required><button class="ui-button ui-button--secondary">Submit Refund Return</button></form></div>
