...
<div class="surface-panel p-4 mt-4"><h5>Request a Refund Return</h5><p class="small text-muted">Available within <?php echo return_request_window_days(); ?> calendar days of confirmed delivery.</p><form method="post" action="/customer/request-return.php" enctype="multipart/form-data"><?php echo csrf_field(); ?><input type="hidden" name="order_id" value="<?php echo $orderId; ?>"><?php foreach ($items as $item): ?><label class="form-label small"><?php echo e((string) ($item['fabric_name_snapshot'] ?? 'Product')); ?> quantity</label><input class="form-control mb-2" aria-label="Return quantity for <?php echo e((string) ($item['fabric_name_snapshot'] ?? 'product')); ?>" type="number" step="<?php echo ($item['unit_type'] ?? 'meter') === 'meter' ? '0.01' : '1'; ?>" min="0" max="<?php echo e((string) (($item['quantity'] ?? 0) > 0 ? $item['quantity'] : $item['quantity_meters'])); ?>" name="return_qty[<?php echo (int) $item['id']; ?>]" value="0"><?php endforeach; ?><select class="form-select mb-2" aria-label="Return reason" name="reason" required><option value="">Reason</option><option>Damaged Item</option><option>Wrong Item Delivered</option><option>Quality Not as Expected</option><option>Other</option></select><textarea class="form-control mb-2" aria-label="Optional return note" name="customer_note" placeholder="Optional note"></textarea><input class="form-control mb-2" aria-label="Required return image 1" type="file" name="image_1" accept="image/jpeg,image/png,image/webp" required><input class="form-control mb-2" aria-label="Required return image 2" type="file" name="image_2" accept="image/jpeg,image/png,image/webp" required><button class="btn btn-outline-secondary">Submit Refund Return</button></form></div>
